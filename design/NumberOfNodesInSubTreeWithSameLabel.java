...
class Solution {
    ...
    private int[] dfs(int parent, int node, Map<Integer, List<Integer>> adj, Map<Integer, Character> idx, int[] ans) {
        int[] nodeCounts = new int[26]; 
        nodeCounts[idx.get(node)-'a'] = 1; 

        if (!adj.containsKey(node)) return nodeCounts; 

        for (Integer child : adj.get(node)) { 
            if (child == parent) continue; 
            int[] childCounts = dfs(node, child, adj, idx, ans);

            for (int i=0; i<26; i++) nodeCounts[i] += childCounts[i]; 
        }

        ans[node] = nodeCounts[idx.get(node) - 'a']; 
        return nodeCounts;
    } 
}
